...
item* insert(item* the_first, char* the_data){
    /*****************************************
...
item* find(item* an_item, char* looking_for){
    /*********************************************************************
    ...
    int result = strcmp(an_item->data, looking_for);

    if(result==0){
        return an_item;
    }
    return find(an_item->next, looking_for);
}
